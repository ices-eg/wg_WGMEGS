 ...
 
{
 ...
 print(out)
  write.csv(out, "output/output_table_hom.csv", na="", row.names = F)
}
...
tableDEP2<-
  function(data)
  {
    ...
    out<-rbind.fill(out,(select(tdate,Year, inter_period, Component, dates, datee, days_interper) %>%   rename(period = inter_period, days = days_interper)%>%mutate(dates=NA,datee=NA)))
