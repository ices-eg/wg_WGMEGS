...
megs_data_survey<-dplyr::bind_rows(data,dplyr::mutate_all(read.csv(path, stringsAsFactors=FALSE, strip.white=TRUE,sep=','),as.character))%>%
          ...
          dplyr::filter(declat>=43)%>%       ##select western stock
          
          dplyr::mutate(Raised_HomStage1=tidyr::replace_na(Raised_HomStage1 ,0))%>%     #some NA in hom1 
...
subset(megs_data_survey, Year==year )->subset_name
assign(paste("survey_megs_HOM",year, sep="_"),subset_name)%>%droplevels()

rm(subset_name, data)
#print(summary(subset_name))
print(paste("NOTE!!!! New file: survey_megs_HOM",year, sep="_"))
