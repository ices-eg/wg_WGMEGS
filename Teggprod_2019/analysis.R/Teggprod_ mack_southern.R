...
##importing grid data by region. For mackerel Southern component area (https://github.com/GersomCostas/Mack_southern-component-grid)

 load("data/AEPM_grid_mack_Southern.RData")

# view survey grid for mackerel Southern component

southern_grid(RECT_p)

# if you want: Save survey grid for mackerel southern component into figures folder

#southern.grid_save(RECT_p)
...
summary(survey_megs_S_2019)

#date summary by period

dateperiod(survey_megs_S_2019)
...
survey_megs_S_2019<-daily_eggprod(survey_megs_S_2019)
...
survey_megs_S_2019.ls<-ls_spat(survey_megs_S_2019)


##Checking  period dates in spatial object (".id" means period, dates: start date, datee: end date )

ldply(survey_megs_S_2019.ls, function(x)data.frame(dates = min(x@data$Date), datee = max(x@data$Date)))
...
plothaul_view_s(survey_megs_S_2019.ls)


##If you like save the plot into images folder. Named as "survey"

plothaul_save_s(survey_megs_S_2019.ls)
...
survey_megs_S_2019.lspdf<-overlap.grid(survey_megs_S_2019.ls)
...
survey_megs_S_2019.lspdf<-interpol(survey_megs_S_2019.lspdf)
...
survey_megs_S_2019.lspdf<-eggprod(survey_megs_S_2019.lspdf)
...
cv_traditional<- coef_var_trad(survey_megs_S_2019.ls)
cv_gam<- cv_gam(survey_megs_S_2019.ls)
#cv_gam<-1.511562 #mistake in gam , then we used all cv gam in western
##VARIANCE in rectangles. the variance in egg densities for each sampling rectangle.

survey_megs_S_2019.lspdf<-var_mack(survey_megs_S_2019.lspdf)
...
tabDEP<-tableDEP(survey_megs_S_2019.lspdf)

#saveRDS()

tabDEP_midday<-tableDEP2(survey_megs_S_2019.lspdf)
...
MACKSsouth_2019_plane<-ldply(survey_megs_S_2019.lspdf,function(x)data.frame(x@data))
  
head(MACKSsouth_2019_plane)

write.csv(MACKSsouth_2019_plane,"output/south_df2019.csv", row.names=F)
...
#ssbs <- t(t(taep.mack) / Frw * 2 * 1.08) * 1e-9
...
SSBS<-(TAEP/Frw * 2 * 1.08) * 1e-9


#Variance (biomass) = Biomass2 x (CV(E)2 + CV(F)2)
VarSSB<-(SSBS* 1e9)^2*((sqrt(varTAEP)/TAEP)^2+(sqrt(varFrw)/Frw)^2)

Final_table<-matrix(c(TAEP,varTAEP,Frw, varFrw,SSBS,  VarSSB), nrow=2) 
colnames(Final_table)<-c("TAEP","Frw","SSBS")
...
write.csv(Final_table,"output/Final_table_southern_2019_25.04.20.csv",row.names = T,na="NA")
