...
source("scripts_hom/00_install.R")
...
load("data/HOM_grid_Western.RData")
...
file_name="data/Gersom 2019 data 21-08-2019"

#name of date file. Without extension.

file_date="data/dates_MEGS_2019_21.08.2019_New_HOM"
...
source("scripts_hom/01_Import_data.R")
...
summary(survey_megs_HOM_2019)

#date summary by period

dateperiod(survey_megs_HOM_2019)
##ad-hoc
survey_megs_HOM_2019<-survey_megs_HOM_2019%>%mutate(Period=replace(Period, Date>="2019-04-11"&Date<"2019-04-15",3))
dateperiod(survey_megs_HOM_2019)
...
survey_megs_HOM_2019<-daily_eggprod(survey_megs_HOM_2019)
...
survey_megs_HOM_2019.ls<-ls_spat(survey_megs_HOM_2019)


##Checking  period dates in spatial object (".id" means period, dates: start date, datee: end date )

ldply(survey_megs_HOM_2019.ls, function(x)data.frame(dates = min(x@data$Date), datee = max(x@data$Date)))
...
plothaul_view_hom(survey_megs_HOM_2019.ls)


##If you like save the plot into images folder. Named as "survey"

#plothaul_save_hom(survey_megs_HOM_2019.ls)
...
survey_megs_HOM_2019.lspdf<-overlap.grid(survey_megs_HOM_2019.ls)
...
survey_megs_HOM_2019.lspdf<-interpol(survey_megs_HOM_2019.lspdf)
...
survey_megs_HOM_2019.lspdf<-eggprod(survey_megs_HOM_2019.lspdf)
...
cv_traditional<- coef_var_trad(survey_megs_HOM_2019.ls)
cv_gam<- cv_gam(survey_megs_HOM_2019.ls)

##VARIANCE in rectangles. the variance in egg densities for each sampling rectangle.

survey_megs_HOM_2019.lspdf<-var_hom(survey_megs_HOM_2019.lspdf)
...
tabDEP<-tableDEP(survey_megs_HOM_2019.lspdf)

saveRDS()


tabDEP_midday<-tableDEP2(survey_megs_HOM_2019.lspdf)
...
HOMt_2019_plane<-ldply(survey_megs_HOM_2019.lspdf,function(x)data.frame(x@data))
  
head(HOMt_2019_plane)

write.csv(HOMt_2019_plane,"output/HOM_df2019.csv", row.names=F)
